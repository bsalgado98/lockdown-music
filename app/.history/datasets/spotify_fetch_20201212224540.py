...
def build_artist_genres(data):
    ...
    for row in data:
        ...
        narrowed_genre = narrow_genre(genres)
        print(f'{artist}: {genres} => {narrowed_genre}')
        result[artist] = narrowed_genre
        counter += 1
    return result

def narrow_genre(genres):
    acceptable_genres = ['r&b', 'rap', 'jazz', 'rock', 'folk', 'indie', 'pop', 'country', 'soul', 'dance']
