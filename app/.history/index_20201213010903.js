...
function genresChart(jsonData) {
    ...
    const grammysGenresCount = {}
    for (genre of Object.values(jsonData)) {
        if (grammysGenresCount[genre]) {
            grammysGenresCount[genre] += 1
        }
        else {
            grammysGenresCount[genre] = 1
        }
    ...
    for (const [genre, count] of Object.entries(grammysGenresCount)) {
        grammysGenresData.push({
            label: genre,
